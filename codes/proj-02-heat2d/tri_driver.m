...
tri_n_el   = 2 * n_el_x * n_el_y; % total number of elements
...
% IEN array
tri_IEN = zeros(tri_n_el, n_en);
for ex = 1 : n_el_x
  for ey = 1 : n_el_y
    ee = (ey-1) * n_el_x + ex; % element index
    tri_IEN(ee * 2 - 1, 1) = (ey-1) * n_np_x + ex;
    tri_IEN(ee * 2 - 1, 2) =  ey * n_np_x + ex + 1;
    tri_IEN(ee * 2 - 1, 3) =  ey * n_np_x + ex ;
    tri_IEN(ee * 2, 1) = (ey-1) * n_np_x + ex;
    tri_IEN(ee * 2, 2) = (ey-1) * n_np_x + ex + 1;
    tri_IEN(ee * 2, 3) =  ey    * n_np_x + ex + 1;
  end
end
...
for ee = 1 : tri_n_el
  x_ele = x_coor( tri_IEN(ee, 1:n_en) );
  y_ele = y_coor( tri_IEN(ee, 1:n_en) );
  ...
  for ll = 1 : n_int
    ...
    for aa = 1 : n_en %get each quadrature point's x = xieNi; take (-1,-1) (1,-1) (1,1) (-1,1) when aa = 1,2,3,4
      %Triangle means the 1-order shape function on triangle
      x_l = x_l + x_ele(aa) * Triangle(aa, xi(ll), eta(ll)); %since the property of the node exactly, this x_l means the ture x coordinate of the integral point
      y_l = y_l + y_ele(aa) * Triangle(aa, xi(ll), eta(ll));    
      %Triangle_grad means the 1-order shape function's derivative on triangle
...
tri_disp = zeros(n_np, 1); %here 'disp' means displacement

for ii = 1 : n_np
  index = ID(ii);
  if index > 0
    tri_disp(ii) = tri_dn(index);
  else
...
for ee = 1 : tri_n_el
    x_ele = x_coor(tri_IEN(ee, :));
    y_ele = y_coor(tri_IEN(ee, :));
    d_ele = tri_disp(tri_IEN(ee, :));
